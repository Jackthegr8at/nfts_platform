...
import { createSpecialMintService } from '@services/template/createSpecialMintService';
...
function NewSpecialMint({
  ...
  const [isSpecialMint, setIsSpecialMint] = useState(false);
  const [isBurnable, setIsBurnable] = useState(true);

  const handleTransferableChange = (value) => {
    if (value || isBurnable) {
      setIsTransferable(value);
    }
  };

  const handleBurnableChange = (value) => {
    if (value || isTransferable) {
      setIsBurnable(value);
    }
  };

  const handleSpecialMintChange = (value) => {
    setIsSpecialMint(value);
  };
  ...
  function handleSetSchemasAttributes(schemaName: string) {
    ...
    let schemasAttributes = schema?.format ?? [];
  ...
  async function onSubmit({
    ...
    try {
      ...
      const selectedSchemaName = selectedSchema ? selectedSchema.schema_name : schemaName;
      ...
      const createService = isSpecialMint ? createSpecialMintService : createTemplateService;
      await createService({
          ...
          <div className="w-full flex flex-col gap-16">
            <div className="flex max-w-3xl flex-col gap-8">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Switch
                    ...
                    onChange={(value) => {
                      handleTransferableChange(value);
                      setValue('transferable', value);
                    }}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Switch
                    ...
                    onChange={(value) => {
                      handleBurnableChange(value);
                      setValue('burnable', value);
                    }}
                    checked={isBurnable}
                  />
                )}
              ...
              <p className="body-1 text-neutral-400 max-w-3xl mb-8">
                To prevent getting stuck with an NFT, it's been ensured
                that both switches cannot be turned off at the same time.
              </p>
              <Input
                {...register('maxSupply')}
                type="text"
              ...
              <div className="flex-none">
                <Popover className="relative">
                  <Popover.Button
                    className={"btn btn-square btn-small btn-ghost opacity-50"}
                  >
                    <Info size={24} />
                  </Popover.Button>
                  <Popover.Panel className="w-64 p-4 z-10 bg-neutral-700 top-12 right-0 rounded">
                    {<p className="body-3">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Switch
                    label="Mint 1 NFT"
                    onChange={(value) => {
                      handleSpecialMintChange(value);
                      setValue('specialmint', value);
                    }}
                    checked={isSpecialMint}
                  />
                )}
              />
              <p className="body-1 text-neutral-400 max-w-3xl mb-8">
                Enabling this toggle will initiate the immediate minting of
                one NFT using the SPECIALMINT contract at a cost of 0.01$ USDC.
                For minting multiple NFTs, it's more cost-effective to use the
                NFT tab after creating the template.
              </p>
            </div>
                    ...
                    <div className="col-span-12 sm:col-span-6 lg:col-span-6 xl:col-span-6">
                      {schemaAttribute.type === 'image' ? (
                        <InputPreview
                          ...
                          accept="image/*"
                        />
                      ) : (schemaAttribute.name === 'image' && schemaAttribute.type === 'string') ? (
                        <InputPreview
                          {...register(schemaAttribute.name, {
                            onChange: () => {
                              handleSetImmutableAttributes({
...
export default withUAL(NewSpecialMint);
